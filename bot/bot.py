...
access_token, guild_id, imdb_file = conf_parser.parse_credentials_form_config(
...
async def save_field_value(interaction: discord.Interaction, value, field_type: str, update: bool):
    ...
    if all(item != None for item in data_store.values()):
        if data_store.get('title', []):
            print(
                f"DEBUG: Title field set [UpdateMode: {update}]- check if user's list contains {data_store['title']}")
            item_in_users_list = manager.get_user_movie_by_title(
                interaction.user.name, data_store['title'])
            # User has an entry of the same movie
            update = True if item_in_users_list else update
...
class ModularView(ui.View):
    ...
    async def interaction_guard(self, interaction: discord.Interaction):
        if (interaction.user != self.initiator):  # Interaction guard
            print(
                f"DEBUG: Interaction guard - BLOCK - user: {interaction.user.name} owner: {self.initiator.name}")
...
class ButtonView(ui.View):
    ...
    async def interaction_guard(self, interaction):
        if (interaction.user != self.initiator):  # Interaction guard
            print(
                f"DEBUG: Button Interaction guard - BLOCK - user: {interaction.user.name} owner: {self.initiator.name}")
            await interaction.response.send_message("You are not the owner of this interaction, wait for your turn.", ephemeral=True)
            return True
    ...
    async def on_submit_press(self, interaction: discord.Interaction, button):
        if not await self.interaction_guard(interaction):
            ...
            if self.update:
                ...
                manager.write_obj_to_collection(
                    {**data_store, **{'user': f"{interaction.user.name}"}})
...
class MovieForm(ui.Modal, title='Questionnaire Response'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        display_me.children[0].options = [discord.SelectOption(
            label=f"{item}") for item in filtered_list]
...
async def call_db_diff_update():  # TODO: Refactor
    filtered = {k: v for (k, v) in diff_store.items()
                if k not in ['_id', 'user']}
    data_diff = {k: data_store[k]
                 for k in data_store if k in data_store and data_store[k] != filtered[k]}
    manager.update_user_movie_by_title(
        diff_store['user'], diff_store['title'], data_diff)
...
async def get_movies_of(ctx, user):
    ...
    server_users = [m.name async for m in ctx.guild.fetch_members(limit=None)]
...
async def testmodal(interaction: discord.Interaction):
    if blocklist_interaction_gurad(interaction.user.name):  # Guard
        # Ephemeral does work on interaction
        await interaction.response.send_message("You have been blocked by an admin", ephemeral=True)
        return
    # TODO: Align with frontend (MOVEME)
    if manager.get_user_item_count(user=interaction.user.name) == 10:
...
if __name__ == '__main__':
    ...
    BOT_ACCESS_BLOCKED = [user['name']
                          for user in manager.blocklist_get_current()]
